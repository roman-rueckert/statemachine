...
/*void aendern() {
    ...
    // Use a map to store the "Regale" information
    std::map<int, std::vector<std::string>> regale;
    for (const auto& change : mein_json["Regale"]) {
        regale[change["Regal"]] = change["Inhalt"];
    }

    // Find the regal with the specified number
    auto it = regale.find(regalnummer);
    if (it != regale.end()) {
        // Use a sorted vector to store the "Inhalt" information
        std::vector<std::string>& inhalt = it->second;
        std::sort(inhalt.begin(), inhalt.end());

        std::cout << "Das Regal " << it->first << " beinhaltet folgende dinge: ";
        for (const auto& item : inhalt) {
            std::cout << item << " ";
        }
        std::cout << "\n";
        ...
        auto it2 = std::lower_bound(inhalt.begin(), inhalt.end(), alterinhalt);
        if (it2 != inhalt.end() && *it2 == alterinhalt) {
            std::cout << *it2 << "\n"; 
